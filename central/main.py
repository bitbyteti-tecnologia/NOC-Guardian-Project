...
async def ingest_telemetry(data: Dict, authorization: Optional[str] = Header(None), request: Request = None):
    ...
    Este endpoint implementa a camada de transporte seguro (Secure Communication Layer).
    
    Args:
        data (Dict): Payload JSON contendo campo 'payload' criptografado.
    ...
    # Extração do Payload Criptografado
    encrypted_b64 = data.get("payload")
    if not encrypted_b64:
        raise HTTPException(status_code=400, detail="Missing 'payload' field")

    try:
        # Utiliza função auxiliar de descriptografia
        telemetry_data = decrypt_payload(encrypted_b64)

        # Log de Sucesso (Apenas Metadados)
        # [TODO]: Futuramente acoplar aqui a verificação mTLS (Client Certificate)
        node_id = telemetry_data.get("node_id", "UNKNOWN")
        logger.debug(f"[SECURE INGEST] Dados recebidos e descriptografados com sucesso de {node_id}")

        # Processamento (Aqui salvaria no banco de dados TimeScaleDB)
        # Por enquanto, apenas confirmamos o recebimento.
        
        return {"status": "received", "bytes_processed": len(encrypted_b64)}

    except InvalidTag:
        logger.warning("[ALERTA DE SEGURANÇA] Falha na descriptografia: Assinatura inválida (Chave errada ou dados adulterados).")
